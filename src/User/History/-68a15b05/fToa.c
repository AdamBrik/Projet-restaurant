...
int main(int argc, char *argv[]) {
    ...
    while(1) {
        ...
        if (nb_lus > 0) {
            ...
            if (lieu_trouve) {
                // Le lieu est trouvé, on ouvre son fichier de menu !
                FILE *fichier_menu = fopen(nom_fichier_menu, "r");
                if (fichier_menu != NULL) {
                    ...
                    if (fgets(buffer_lecture, sizeof(buffer_lecture), fichier_menu) != NULL) {
                        ...
                        snprintf(rep.message, sizeof(rep.message), "Le fichier menu est vide.");
                    }
                    fclose(fichier_menu);
                } else {
                    snprintf(rep.message, sizeof(rep.message), "Erreur: Fichier menu %s introuvable.", nom_fichier_menu);
                }
            } else {
