...
int main(int argc, char *argv[]) {
    ...
    while(1) {
        ...
        if (nb_lus > 0) {
            ...
            if (lieu_trouve) {
                // On ouvre directement le fichier indiqué dans config.txt (ex: "menu_1.txt")
                ...
                if (fichier_menu != NULL) {
                    ...
                    snprintf(rep.message, sizeof(rep.message), "[%s] Erreur: Fichier %s introuvable.", nom_resto, nom_fichier_menu);
                }
            } else {
