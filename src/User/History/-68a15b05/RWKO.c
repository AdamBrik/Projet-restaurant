...
int main(int argc, char *argv[]) {
    ...
    while(1) {
        ...
        if (nb_lus > 0) {
            // ÉTAPE DE VALIDATION DU MESSAGE
            ...
            rep.id_client = cmd.id_client; // On garde l'ID pour le routeur
            rep.statut = 0; 
            ...
            if (config != NULL) {
                // On cherche le lieu dans config.txt
            ...
            if (lieu_trouve) {
                char nom_fichier_final[150];
                snprintf(nom_fichier_final, sizeof(nom_fichier_final), "%s_%04d.txt", nom_fichier_menu, cmd.id_plat);

                FILE *fichier_menu = fopen(nom_fichier_final, "r");
                ...
                if (fichier_menu != NULL) {
                    ...
                    snprintf(rep.message, sizeof(rep.message), "[%s] Erreur: Le menu %04d est indisponible.", nom_resto, cmd.id_plat);
                }
            } else {
